...
def simulate_for_M(M, N, incoming_data):
    ...
    Returns:
    float: The percentage of unordered entries.
    """
    ...
    return sppifoq_outgoing_data, sppifo_unordered_count / len(incoming_data) * 100
...
if __name__ == "__main__":
    ...
    x, y = simulate_for_M(2, N, incoming_data)  # Run the simulation for M=1 to establish a baseline
    print(f"Incoming data: {incoming_data}")  # Print the generated incoming data
    print(f"Outgoing data: {x}")  # Print the unordered entries percentage for M=1
    print(f"Unordered entries percentage for M=1: {y:.2f}%")  # Print the unordered entries percentage for M=1
